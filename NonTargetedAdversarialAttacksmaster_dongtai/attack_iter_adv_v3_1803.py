"""实现adv_v3给出对抗样本的标签 大的置信度模型给与大的权重"""
...
tf.flags.DEFINE_string(
    'output_dir', './out_images_v1', 'Output directory with images.')
...
def main(_):
    ...
    # x_adv, y, _, _, _, _, pred = tf.while_loop(stop, graph, [x, y, i, x_max, x_min, grad, pred])
        ...
        for filenames, images in load_images(FLAGS.input_dir, batch_shape):
            ...
            prediction_adv_v3 = sess.run(pred_adv_v3, feed_dict={x_adv_v3: images})
            print('prediction_adv_v3::::::', prediction_adv_v3)
            for l in range(len(filenames)):
                label_distance += abs(prediction[l] - prediction_adv_v3[l])
                if int(prediction[l]) == prediction_adv_v3[l]:  # The test tag is always 1 more than the real tag, so I subtract 1 here
                    loss_num += 1
