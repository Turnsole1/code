            ...
            for i in range(FLAGS.num_iter):
                ...
                if i == FLAGS.num_iter - 1:
                    ...
                    with slim.arg_scope(resnet_v2.resnet_arg_scope()):
                        logits_resnet, end_points_resnet = resnet_v2.resnet_v2_101(
                            x, num_classes=num_classes, is_training=False)
                    pred_dict = end_points_v3['Predictions'] + end_points_adv_v3['Predictions'] + \
                                end_points_ens3_adv_v3['Predictions'] + \
                                end_points_ens4_adv_v3['Predictions'] + end_points_v4['Predictions'] + \
                                end_points_res_v2['Predictions'] + end_points_ensadv_res_v2['Predictions'] + \
                                end_points_resnet['predictions']

                    adv_prediction = tf.argmax(pred_dict, axis=1)

                    adv_prediction = sess.run(adv_prediction, feed_dict={x: images})

                    print('adv_prediction:::::::', adv_prediction)


                    for l in range(len(filenames)):
                        label_distance += abs(prediction[l] - adv_prediction[l])

                        if int(prediction[l]) == adv_prediction[l]:  # The test tag is always 1 more than the real tag, so I subtract 1 here
                            success_num += 1
