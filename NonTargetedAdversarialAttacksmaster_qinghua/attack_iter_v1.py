...
tf.flags.DEFINE_integer(
    'num_iter', 45, 'Number of iterations.')
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    x_adv, label, _, _, _, _, Pred = tf.while_loop(stop, graph, [x_input, y, i, x_max, x_min, grad, pred])
    ...
    with tf.Session() as sess:
      ...
      for filenames, images in load_images(FLAGS.input_dir, batch_shape):
        ...
        adv_images, Prediction, true_label= sess.run([x_adv, Pred, label], feed_dict={x_input: images})
